...
public class Parser extends Thread {
	...
	void buildtree() {
		...
		while (true) {
			...
			if (token.type.equals("end")) {
				...
				// paired end tag
				if (stack.size() > 0 && stack.peek().value.equals(token.value))
				...
				// unpaired end tag
					...
					// if root is not empty, add it to forest, create a new root
					if (root.children.size() > 0) {
						remain.forest.add(root);
						root = new Node(null, null, true);
						currentNode = root;
						// the treeStack and stack could be nonempty caused by
						// unpaired tags in broken comments
						// if(treeStack.size() > 0)
			...
			if (token.type.equals("comment_end")) {
				System.out.println(getName() + " rollbacks from " + pos
						+ " to 0, reprocesses its segment.");
				// rollback
				pos = 0;
	...
	Token tokenize() {
		...
		if (tokenStr.startsWith("<!--")) {
			...
			while (tokenStr.endsWith("-->") == false
					&& tokenStr.equals("EOF") == false) {
				tokenStr = read();
				value += tokenStr;
			}
			if (tokenStr.endsWith("-->"))
				return new Token("comment", value.substring(4,
						value.length() - 3));
			else
				return new Token("comment_start", value.substring(4,
						value.length()));
		}
	...
	public void run() {
		...
		// treePrintToDot(remain.forest.get(i),
		// "dot/"+getName()+"tree"+i+".dot");
		// }
	}
