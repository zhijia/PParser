...
public class PostProcessor {
	...
	Remain merge(Remain remain0, Remain remain) {
		...
		if (rightMost.token.type.equals("comment_start")
			...
			if (leftMost.token.type.equals("comment")) {
				...
				// add leftMost's comment to rightMost's comment
				rightMost.token.value += leftMost.token.value;

				// remove leftMost
				leftMost.parent.children.remove(0);
				...
				if (node.children.size() == 0) // single end tag
				{
					if (node.token.value.equals(rightEdge.get(j).token.value))
						...
						System.out.println("tree merge err: "
								+ rightEdge.get(j).token.value + " not match "
								+ node.token.value);
					break;
	...
	// get an array of nodes appeared on the right edge of a tree
